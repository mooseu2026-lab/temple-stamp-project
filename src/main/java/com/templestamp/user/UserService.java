...
import com.templestamp.auth.AuthService;
...
public class UserService {
    ...
    /**
     * 회원 탈퇴. 약관 철회(withdraw)와 이름을 나눈 이유는 둘이 전혀 다른 일이기 때문이다 —
     * 하나는 되돌릴 수 있고 하나는 되돌릴 수 없다.
     * <p>
     * 비밀번호 재확인은 <b>연쇄 트랜잭션 밖에서</b> 먼저 한다. 안에서 하면 실패 횟수를 올린 UPDATE 가
     * 같은 트랜잭션의 예외에 휩쓸려 사라져 5회 잠금이 걸리지 않는다(정리.md §6-2).
     * <p>
     * {@code NOT_SUPPORTED} 인 이유가 그것이다. 이 클래스는 {@code @Transactional(readOnly = true)} 라
     * 그냥 두면 <b>읽기 전용 트랜잭션</b>이 바깥에 열리고, 안쪽 쓰기가 "Connection is read-only" 로 죽는다.
     * 여기서는 트랜잭션을 아예 열지 않고, 재확인과 연쇄가 <b>각자</b> 자기 트랜잭션을 연다.
     */
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
    public void deleteAccount(Long userId, String password) {
        authService.reauthenticate(userId, password);
    ...
    /**
     * 기본은 ACTIVE 다. 탈퇴 계정은 {@code status=DELETED} 를 <b>명시할 때만</b> 나온다 —
     * 기본으로 섞이면 관리자가 탈퇴한 사람에게 연락하거나 처리를 시도하는 일이 생긴다.
     * 비밀번호 해시·토큰·주소는 질의 자체가 읽지 않는다(응답에서 지우는 방식이 아니다).
     */
    public com.templestamp.global.response.PageResponse<com.templestamp.admin.dto.AdminUserResponse>
            listForAdmin(String status, String q, int page, int size) {
        String normalized = status == null || status.isBlank()
                ? "ACTIVE" : status.trim().toUpperCase(java.util.Locale.ROOT);
        if (!"ACTIVE".equals(normalized) && !"DELETED".equals(normalized) && !"ALL".equals(normalized)) {
            throw new BusinessException(ErrorCode.COMMON_4000, "status 는 ACTIVE·DELETED·ALL 중 하나입니다.");
        }
        String filter = "ALL".equals(normalized) ? null : normalized;
        String keyword = q == null || q.isBlank() ? null : q.trim();

        long total = userMapper.countForAdmin(filter, keyword);
        var items = userMapper.findForAdmin(filter, keyword, page * size, size);
        return com.templestamp.global.response.PageResponse.of(items, page, size, total);
    }
}