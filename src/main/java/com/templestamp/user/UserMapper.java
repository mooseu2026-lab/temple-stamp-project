...
public interface UserMapper {
    ...
    /**
     * 탈퇴 — 행을 지우지 않고 개인정보만 지운다(챕터 1 보강 §3).
     * 이메일을 `deleted-{id}@invalid` 로 바꾸는 이유가 둘이다: 유니크를 비워 같은 이메일로 재가입할 수 있게,
     * 그리고 `@invalid` 는 예약된 최상위 도메인이라 실제로 메일이 나갈 수 없게.
     */
    int anonymize(@Param("userId") Long userId);

    /** 필터가 매 요청 확인한다. 탈퇴한 사용자의 액세스 토큰은 만료 전이라도 통하면 안 된다. */
    String findStatus(@Param("userId") Long userId);

    /* ---------------- 관리자 목록 (챕터 9 §6) ---------------- */

    /** filter 가 null 이면 전부. 비밀번호 해시는 <b>질의가 읽지 않는다</b>. */
    java.util.List<com.templestamp.admin.dto.AdminUserResponse> findForAdmin(
            @Param("status") String status, @Param("q") String q,
            @Param("offset") int offset, @Param("limit") int limit);

    long countForAdmin(@Param("status") String status, @Param("q") String q);
}