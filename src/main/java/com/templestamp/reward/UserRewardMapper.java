...
public interface UserRewardMapper {
    ...
    /** 탈퇴 — 아직 아무도 움직이지 않은 적립분만 회수한다(§2-4 와 같은 규칙). */
    int revokeGrantedByUser(@Param("userId") Long userId);

    /** 탈퇴 — 신청·심사·지급까지 간 것은 상태를 두고 표시만 켠다. 실물이 이미 나갔을 수 있다. */
    int flagNeedsReviewByUser(@Param("userId") Long userId);
