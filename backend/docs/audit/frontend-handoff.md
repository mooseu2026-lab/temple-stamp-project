...
### 공양 — 값이 곧 안내다 (2026-09-08 예성 확인 · C1 보강)

`site.mealAvailable` 은 값 4종이다.

| 값 | 화면 문구 | 뜻 | 지금 |
|---|---|---|---:|
| `TEMPLE_MEAL` | **사찰음식 체험 — 사전 예약 필요** | 사찰음식으로 이름난 곳 | **6곳** |
| `RESERVATION` | **공양 가능 — 방문 전 종무소에 시간·예약 문의** | **기본값** | **109곳** |
| `NONE` | **공양 없음** | 없음이 *확인된* 곳 | 0곳 |
| `NEARBY` | **경내 공양 없음 · 인근 식당 이용** | 확인된 곳만 | 0곳 |

> **서버는 값만 준다. 문구는 프론트가 매핑한다**(i18n 대상 · `frontend-design-v2.1.md` §21-1 `MEAL_LABEL`).

**왜 값 이름에 "예약" 이 들어가는가.** 대부분의 절에 공양이 있지만 **시간이 정해져 있고 미리 연락해야 한다.**
값을 "있다/없다" 로만 두면 화면이 "공양 가능" 이라고만 띄우게 되고, 시간을 모르고 찾아간 사람이 헛걸음을 한다.
`RESERVATION` 은 "있다" 가 아니라 **"있는데 예약이 필요하다"** 는 뜻이다 — 그 사실을 값이 직접 나른다.

- **`NULL` 도 빈칸도 없다.** 115곳 전부 값이 있다. "모른다" 로 둘 이유가 없다 —
  모르는 것은 *얼마나 미리 연락해야 하는가* 이지 *있는가* 가 아니고, 그 모름은 `RESERVATION` 이 이미 말한다.
- **절별 공양 시간은 아직 조사되지 않았다**(`checkedAt` 이 비어 있는 이유). 확인되면 사찰별 안내로 바꾼다.
- `NONE`·`NEARBY` 는 아직 한 곳도 없지만 값 체계에 남긴다 — 확인되는 대로 들어온다.
