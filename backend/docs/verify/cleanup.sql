...
-- USE 를 두지 않는다. 부르는 쪽이 mysql -D 로 스키마를 정한다 —
-- 그래야 같은 파일로 개발 DB 와 배포 리허설용 빈 DB 를 함께 돌릴 수 있다(최종 점검 F §3-1).

-- ── 챕터 9 — 전자책·인쇄 주문·파일 삭제 큐 (맨 먼저) ──────────────────
-- 순서가 정해져 있다: print_order_address → print_order → ebook → storage_orphan.
-- 주소는 주문을, 주문은 전자책을, 전자책은 사용자를 참조한다. 거꾸로 지우면 FK 1451 로 멈춘다.
-- 그래서 계정을 지우는 아래 블록들보다 <b>먼저</b> 온다.
--
-- 계정 패턴이 아니라 표를 통째로 비우는 이유: 이 셋에는 검증이 만드는 것 말고 들어갈 것이 없다
-- (아직 운영 전이고, 완주가 만드는 코스본도 검증이 만든 완주의 산물이다).
-- 일부만 지우면 다른 계정의 주문이 참조하는 전자책에서 막힌다 — 실제로 그렇게 한 번 멈췄다.
--
-- 파일 실물(EBOOK/·CERT/)은 run-all.sh 가 지운다 — SQL 로는 디스크를 건드릴 수 없다.
DELETE FROM print_order_address;
DELETE FROM print_order;
DELETE FROM ebook;
DELETE FROM storage_orphan;
...
-- 챕터 1 보강(탈퇴): 탈퇴한 계정은 이메일이 deleted-{id}@invalid 로 바뀌어 있어
-- 원래 이메일로는 찾을 수 없다. 검증 DB 에는 진짜 탈퇴 회원이 없으므로 이 조건으로 지운다.
-- ★ 운영에는 이 파일을 돌리지 않는다(검증 전용).
DELETE FROM storage_orphan;   -- 탈퇴가 적재한 저장소 키 큐. 검증 DB 에서는 전부 검증 데이터다

DELETE FROM reward_claim
 WHERE user_reward_id IN (SELECT user_reward_id FROM user_reward
                           WHERE user_id IN (SELECT user_id FROM users WHERE email LIKE 'deleted-%@invalid'));
...
DELETE FROM refresh_token
 WHERE user_id IN (SELECT user_id FROM users WHERE email LIKE 'deleted-%@invalid');
DELETE FROM users WHERE email LIKE 'deleted-%@invalid';
-- 챕터 7(W·W2 폴더) + 동시성 점검 계정(conc-a·conc-b) + 보안 점검 계정(sec-check).
-- FK 를 거스르지 않게 위에서부터 지운다.
...
-- 챕터 8 — N 폴더와 CSV 반입이 만든 원고. 시더의 기본 원고 10편(id 1~10)만 남긴다.
-- ★ 파일 맨 끝인 것이 중요하다. stamp.manuscript_id·ext_manuscript_id 가 RESTRICT 라
--   도장을 지우기 전에 원고를 지우면 FK 1451 로 cleanup 이 통째로 멈춘다(챕터 7 사례).
DELETE FROM manuscript WHERE manuscript_id > 10;
